...
void etherboot(void)
{
	...
	cleanup_net();
	cache_flush();
	asm volatile ("jmp @r0; nop":	/* no output */
...
static inline char highhex(int x)
{
	return hexchars[(x >> 4) & 0xf];
}
static inline char lowhex(int x)
{
...
static void setup_parms(void)
{
	...
	*p++ = ' ';
	...
	p = sprintf(p, "root=/dev/nfs rw nfsroot=%I:%s ip=%I::%I:%I:%s::off",
		    ...
		    rootpath,
		    arptable[ARP_CLIENT].ipaddr.s_addr,
		    arptable[ARP_GATEWAY].ipaddr.s_addr, netmask, hostname);
...
static int binaryload(unsigned char *data, int block, int len, int eof)
{
	static long sum;
...
static void load_file(char *name, char *file_name, unsigned long addr)
{
	for (;;) {
...
static int eth_get_node_addr(unsigned char *node_addr)
{
