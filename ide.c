/* $Id: ide.c,v 1.3 2003/12/12 15:39:15 lethal Exp $
 *
 * sh-ipl+g/ide.c
 *
...
void __init init_ide(void)
{
	...
	if ((p4_inw(MRSHPC_CSR) & 0x000c) == 0) {	/* if card detect is true */
...
#define BRIDGE_IDE_INTR_STAT	0x001E
	...
	if ((ide_inw(BRIDGE_FEATURE) & 1)) {	/* We have IDE interface */
		int i;
...
static int ide_identify_device(dev)
{
...
#if !defined(__LITTLE_ENDIAN__)
	/* Fix words for BigEndian CPU - we only use buf[0], buf[3] and buf[6]
	   so don't waist time to swap other bytes */
	buf[0] = ((buf[0] & 0xff) << 8 | (buf[0] >> 8));
	buf[3] = ((buf[3] & 0xff) << 8 | (buf[3] >> 8));
	buf[6] = ((buf[6] & 0xff) << 8 | (buf[6] >> 8));
#endif
...
static int ide_set_transfer_mode(int dev, int mode)
{
	unsigned long status;
	int i;
	...
	status = ide_inb(IDE_STATUS);
	printf("Set Transfer Mode result: %x\n", status);

	return 0;
}

static int ide_set_device_params(int dev, int sectors_per_track, int max_head)
{
	unsigned long status;
	int i;
	...
	status = ide_inb(IDE_STATUS);
	printf("Initialize Device Parameters result: %x\n", status);
 ...
 */
int ide_read_sectors(int dev, unsigned long lba, unsigned char *buf, int count)
{
	/* Select the device */
	if (ide_device_selection(dev) < 0)
		return -1;
