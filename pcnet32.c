...
#define DBPRINT(str) if (pcnet32_debug) printf(str)
...
int eth_reset(unsigned int start_or_stop)
{
	...
	if (!pcdata) {
		...
		if (rc != 0) {
			DBPRINT("In eth_reset(");
			DBPRINT(itox(start_or_stop, 8));
			DBPRINT("): pcnet32_setup_pci() returns");
			DBPRINT(itox(rc, 4));
			DBPRINT("!\n");
			return rc;
		}
...
int eth_startup(void)
{
	...
	if (pcnet32_dwio_read_csr(ioaddr, 0) == 4 && pcnet32_dwio_check(ioaddr)) {
		lp->a = a = &pcnet32_dwio;
	} else if (pcnet32_wio_read_csr(ioaddr, 0) == 4
...
void pcnet32_init_ring(struct pcnet32_data *lp)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		lp->rx_ring[i].base =
		    (u32) le32_to_cpu(&lp->rx_bufs[i][0] + PKT_RESERVE);
		 ...
		 unsigned int size, const char *p)
{
	...
	if (status & 0x8000) {
		...
		DBPRINT(itox(status, 4));
		DBPRINT(", err_status = 0x");
		DBPRINT(itox(err, 8));
		DBPRINT("\n");
	}

	/* Check base address, fix if necessary */
	if (lp->tx_ring[entry].base !=
	    (u32) le32_to_cpu(&lp->tx_bufs[entry][0])) {
...
int eth_receive(char *p, unsigned int *len_p)
{
	...
	while (!rxok && ((short)le16_to_cpu(lp->rx_ring[entry].status) >= 0)) {
		...
		short pkt_len =
		    (le32_to_cpu(lp->rx_ring[entry].msg_length) & 0xfff) - 4;
